...
- One Oban worker can claim adjacent same-Account/scope/session unstamped messages up to a token target. The structured result uses a per-anchor envelope with an explicit `anchor_id` separate from candidate support message ids; every anchor retains its existing deterministic per-message replay key and durable completion state, and every extracted item cites only supplied message ids.
- Persist one anchor's candidates, lifecycle/audit effects, and completion atomically and idempotently. A retry skips completed anchors and processes only incomplete anchors, while a poison anchor remains isolated and siblings remain retryable. Classify failures as retryable (transient provider/network/capacity), repairable (credential/configuration or oversized input requiring operator action), or terminal (non-retryable source-specific poison after bounded structured-repair attempts). Record a content-safe reason; exclude repairable and terminal anchors from normal reconciliation replay; expose an explicit operator repair/requeue action.
- Concurrent workers cannot process one anchor twice into duplicate lifecycle/audit history.
- Config supports at least 128, 1K, 4K, and 16K evaluation variants without adding a new queue table. Each experiment pins the tokenizer and version. Pre-call admission counts the serialized instructions, schema, all source messages/context, and reserved output tokens against the configured provider/model context limit plus a safety margin; provider-reported usage is post-call accounting only. An anchor that cannot fit alone is not truncated silently: record it as repairable `oversized`, then require an explicit requeue under an approved chunking or larger-context policy.

**Gate:** crash/retry/property tests cover failures before and after every anchor commit, mixed-batch anchor/support attribution, two-worker races, failure classification, retry exhaustion, sibling progress, replay exclusion, explicit repair/requeue, pre-call context admission, and a single oversized anchor; compare extraction quality, queue delay, calls, tokens, and cost. Do not select one global batch size unless evidence supports it.
...
- Query embedding is computed once per request and may be cached by Account, embedding identity, and a normalized query digest.
