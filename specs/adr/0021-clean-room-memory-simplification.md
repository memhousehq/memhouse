...
| Authorized source-message embedding/index | No; Message remains truth | `MemHouse.Observations` owns source rows; `MemHouse.Retrieval` owns the read index; the existing coalesced projection job writes it | Message creation transactionally schedules a scope/bucket refresh; retention, scope changes, or erasure affect the canonical rows directly. Reconciliation detects missing or stale four-part identities and rebuilds from surviving messages under a corpus-derived replay key. Export omits vectors. | Delete if source fallback gives no preregistered category gain, cannot meet authorization/erasure tests, or its storage/refresh budget is exceeded. Lexical source search may remain independently if proven. |
