...
defmodule MemHouseWeb.MemoryController do
  ...
  Retrieves supporting memory and answers a natural-language question over it.
    ...
    lineage. `include_source_exact_recall` and `include_source_semantic_recall` select
    source-message tools independently. `include_source_recall: true` remains shorthand
    for both. `include_stable_profile_recall: false` removes profile lookup. Effort alone
    does not permit source-text recall.
