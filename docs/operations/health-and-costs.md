...
node cannot reset its allowances. Aborting consumes the packet with no resume;
recovery requires a newly approved packet, run id, and digest for that exact
`pg0` or `external` backend.
