...
defmodule MemHouse.F2TransactionalWritesAuditJobsTest do
  ...
  test "coalesced projection refresh executes after the ten-second bucket closes" do
    # This test verifies that writes late in a ten-second bucket are included in the refresh
    # rather than joining an already-executed job. The bucket is ten seconds, and execution
    # is scheduled for fifteen seconds after the first write (bucket end plus five).

    account_key = "f2-projection-timing"

    # First write at T=0 creates bucket 0 and schedules refresh for T+15.
    first_at = ~U[2026-08-14 10:00:00.000000Z]

    assert {:ok, first_message} =
             Memory.ingest_message(
               ingest_attrs(account_key, "timing-session", inserted_at: first_at)
             )

    account_id = account_id!(account_key)
    scope_id = first_message.scope_id

    # The first write created a projection_refresh run with the bucket-0 idempotency key.
    DataLayer.with_account_id(account_id, [role: :system, pipeline?: true], fn _account, actor ->
      ...
      expected_key = Idempotency.derived_refresh(scope_id, :projection_refresh, first_at, 10)
      assert first_run.idempotency_key == expected_key
      assert first_run.status == "pending"
    end)

    # Second write at T+9 (same bucket, late in the window) must join the same run.
    # With the old five-second schedule_in, this write would have arrived after the job executed.
    # With the fixed fifteen-second schedule_in, it arrives six seconds before execution.
    late_at = ~U[2026-08-14 10:00:09.000000Z]

    assert {:ok, second_message} =
             Memory.ingest_message(
               ingest_attrs(account_key, "timing-session", inserted_at: late_at)
             )

    assert second_message.scope_id == scope_id

    # The late write reused the same run (same idempotency key for bucket 0).
    DataLayer.with_account_id(account_id, [role: :system, pipeline?: true], fn _account, actor ->
      runs =
        PipelineRun
        |> Ash.Query.filter(kind == "projection_refresh" and target_id == ^scope_id)
      ...
      # Only one run exists for both writes in the same bucket.
      assert length(runs) == 1

      run = Enum.at(runs, 0)
      expected_key = Idempotency.derived_refresh(scope_id, :projection_refresh, late_at, 10)
      assert run.idempotency_key == expected_key
    end)

    # A write in the next bucket (T+10 or later) creates a distinct run.
    next_bucket_at = ~U[2026-08-14 10:00:10.000000Z]

    assert {:ok, third_message} =
             Memory.ingest_message(
               ingest_attrs(account_key, "timing-session-2", inserted_at: next_bucket_at)
             )

    assert third_message.scope_id == scope_id
    ...
    DataLayer.with_account_id(account_id, [role: :system, pipeline?: true], fn _account, actor ->
      ...
      # Now two runs exist: one for bucket 0, one for bucket 1.
      assert length(runs) == 2

      # Verify the second bucket has its own distinct key.
      bucket_0_key = Idempotency.derived_refresh(scope_id, :projection_refresh, first_at, 10)
      bucket_1_key = Idempotency.derived_refresh(scope_id, :projection_refresh, next_bucket_at, 10)

      assert bucket_0_key != bucket_1_key

      keys = Enum.map(runs, & &1.idempotency_key)
      assert bucket_0_key in keys
      assert bucket_1_key in keys
    end)
  end
